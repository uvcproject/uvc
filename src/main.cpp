...
#if defined(NDEBUG)
#error "Uvc cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uuvc) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    static int64_t startamount = 525600;
    if (nHeight == 0) {
        ...
        nSubsidy = 5 * COIN;
    } else if (nHeight >= startamount*2+1 && nHeight <= startamount*3) {
        nSubsidy = 2.5 * COIN;
    } else if (nHeight >= startamount*3+1 && nHeight <= startamount*4) {
        nSubsidy = 1.25 * COIN;
    } else if (nHeight >= startamount*4+1 && nHeight <= startamount*5) {
        nSubsidy = 0.625 * COIN;
    } else if (nHeight >= startamount*5+1 && nHeight <= startamount*6) {
        nSubsidy = 0.3 * COIN;
    } else if (nHeight >= startamount*6+1 && nHeight <= startamount*7) {
        nSubsidy = 0.2 * COIN;
    } else if (nHeight >= startamount*7+1 && nHeight <= startamount*8) {
        nSubsidy = 0.1 * COIN;
    } else {
        nSubsidy = 0 * COIN;
    }
...
void CheckForkWarningConditionsOnNewFork(CBlockIndex* pindexNewForkTip)
{
    ...
    // We use 7 blocks rather uvctrarily as it represents just under 10% of sustained network
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // UVC: We use certain sporks during IBD, so check to see if they are
